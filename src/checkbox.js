  ...
  render() {
    return (
      <div>
        ...
        >
         {this.state.locales.map(l => <ListItem value={l}>{l}</ListItem>)}
        </List>
        {this.state.locales.map(l => <div> <Spinner green /> {l} </div>)}
...
class DownloadTranslation extends Component {
  ...
  renderItem(i) {
    return (
      <ListItem value={this.state.locales[i]}>
        {" "}
  ...
  render() {
    ...
    return <NonInteractiveList locales={this.state.locales} />;
  }
}
